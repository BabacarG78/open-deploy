...
#			Variables JBOSS CLI			   #
#-------------------------------------------------------------------
TEMPLATE_SG="/home/jboss/install/myProject/templateServerGroup.cfg"
DOMAIN_CONFIG_DIR="/usr/local/jboss/domain/configuration"
DOMAIN_CONFIG_FILE=$DOMAIN_CONFIG_DIR"/domain.xml"
SERVER_GROUP_NAME="MYPROJECT_"
OLD_SERVER_GROUP_NAME="MYPROJECT_"
WAR_DIR="/home/jboss/myProject"
RUNTIME_NAME="myProject-webapp.war"
HOST_INSTANCE=""
JBOSS_DEPLOYMENT="/home/jboss/deploy/myProject"
...
#-----------------------------------------------------------------------
LIVRABLE_TEMP_DIR="/home/jboss/myProject"
LIVRABLE="/home/jboss/myProject"
MAVEN_METADATA_SNAPSHOT_DIR="$LIVRABLE_TEMP_DIR/snapshot"
MAVEN_METADATA_RELEASE_DIR="$LIVRABLE_TEMP_DIR/release"
LOG_SCRIPTS="$LIVRABLE_TEMP_DIR/script_install_myProject.log"

#-------------------------------------------------------------------
# 			Variables NEXUS				    #
#-------------------------------------------------------------------
URL_NEXUS="http://s-nexus-qua-01:8081/nexus/content/repositories"
URL_NEXUS_SNAPSHOT="$URL_NEXUS/snapshots/com/de/myProject-webapp"
URL_NEXUS_RELEASE="$URL_NEXUS/releases/com/de/myProject-webapp"
URL_GET_SNAPSHOT="http://s-nexus-qua-01:8081/nexus/service/local/artifact/maven/redirect?r=snapshots&g=com.de&a=myProject-webapp&e=war&v="
URL_GET_RELEASE="http://s-nexus-qua-01:8081/nexus/service/local/artifact/maven/redirect?r=releases&g=com.de&a=myProject-webapp&e=war&v="

#-------------------------------------------------------------------
# 			Variables Pegase			    #
#-------------------------------------------------------------------
MYPROJECT_URL=http://$2/myProject
MYPROJECT_DEST_FILENAME=myProject.html
...
wget -nv -O$LIVRABLE_TEMP_DIR/$MYPROJECT_DEST_FILENAME $MYPROJECT_URL
OLD_VERSION=`cat $LIVRABLE_TEMP_DIR/$MYPROJECT_DEST_FILENAME | grep -oE "<span.*>Version\s:\s.*</span>" | grep -oE "([0-9]|[.])+(-SNAPSHOT)*"`
...
function updateDomain(){
  SERVER_GROUP_NAME=$1
  sed "s/SERVER_GROUP_NAME/$SERVER_GROUP_NAME/" $TEMPLATE_SG > /home/jboss/install/myProject/addServerGroup.xml
  echo "Arret de JBoss ...................................................."
  service jboss stop
  cp -f $DOMAIN_CONFIG_FILE $DOMAIN_CONFIG_DIR"/domain_backup.xml"
  sed -i '/<server-groups>/r /home/jboss/install/myProject/addServerGroup.xml' $DOMAIN_CONFIG_FILE
...
function check_errors(){
		...
		#Delete All servers group and instance
			...
			then 
				printf "undeploy myProject-webapp-$VERSION.war --all-relevant-server-groups\n\n" | tee -a $LOG_SCRIPTS
				./jboss-cli.sh -c --command="undeploy myProject-webapp-$VERSION.war --all-relevant-server-groups" > $JBOSS_DEPLOYMENT/nettoyage_deplyment.log	               
				cat $JBOSS_DEPLOYMENT/nettoyage_deplyment.log
			fi
...
function check_deployment(){
	#Verification des version deployees
	server=$1
	URL="http://$server:$PORT/myProject"
...
printf "Nettoyage de l'ancien livrables myProject-webapp ..................................\n\n" | tee -a $LOG_SCRIPTS
rm -rf $LIVRABLE/*.war

#Telechargement de myProject-webapp
printf "Telechargement du livrable myProject-webapp-$VERSION.war ..................................\n\n" | tee -a $LOG_SCRIPTS
wget -nv -O myProject-webapp-$VERSION.war -P$LIVRABLE_TEMP_DIR $URL_LIVRABLE_DISTANT$VERSION
if [ $? -ne 0 ]
then
	printf "Fichier distant $URL_LIVRABLE_DISTANT/$VERSION/myProject-webapp-$VERSION.war indisponible \n\n" | tee -a $LOG_SCRIPTS
	exit 2
else
	mv myProject-webapp-$VERSION.war $LIVRABLE
fi
...
if [ "$OLD_VERSION" == "$VERSION" ]
then
	printf "Suppression d' myProject-webapp-$VERSION.war du server-group : $OLD_SERVER_GROUP_NAME$OLD_VERSION\n\n" | tee -a $LOG_SCRIPTS
	printf "undeploy myProject-webapp-$VERSION.war --server-groups=$OLD_SERVER_GROUP_NAME$OLD_VERSION\n\n" | tee -a $LOG_SCRIPTS
	./jboss-cli.sh -c --command="undeploy myProject-webapp-$VERSION.war --server-groups=$OLD_SERVER_GROUP_NAME$OLD_VERSION" > $JBOSS_DEPLOYMENT/undeploy.log
	...
	printf "Deploiement d' myProject-webapp-$VERSION.war du server-group $OLD_SERVER_GROUP_NAME$OLD_VERSION \n\n" | tee -a $LOG_SCRIPTS
	printf "deploy $WAR_DIR/myProject-webapp-$VERSION.war --runtime-name=myProject-webapp.war --server-groups=$OLD_SERVER_GROUP_NAME$OLD_VERSION > $JBOSS_DEPLOYMENT/deployment_myProject.log\n\n" | tee -a $LOG_SCRIPTS
	./jboss-cli.sh -c --command="deploy $WAR_DIR/myProject-webapp-$VERSION.war --runtime-name=myProject-webapp.war --server-groups=$OLD_SERVER_GROUP_NAME$OLD_VERSION" > $JBOSS_DEPLOYMENT/deployment_myProject.log
	cat $JBOSS_DEPLOYMENT/deployment_myProject.log | tee -a $LOG_SCRIPTS
	...
	printf "Attente de 60s avant le debut du deploiement myProject-webapp-$VERSION.war........\n\n" | tee -a $LOG_SCRIPTS
	sleep 60 
	printf "#--------------------------------------------------------------------------------------------------\n" | tee -a $LOG_SCRIPTS
	printf "#----------------	       Deploiement de l artefacts : myProject-webapp-$VERSION.war      \n" | tee -a $LOG_SCRIPTS
	printf "#--------------------------------------------------------------------------------------------------\n\n" | tee -a $LOG_SCRIPTS
	printf "deploy $WAR_DIR/myProject-webapp-$VERSION.war --runtime-name=myProject-webapp.war --server-groups=$SERVER_GROUP_NAME_VALUE \n\n" | tee -a $LOG_SCRIPTS
	./jboss-cli.sh -c --command="deploy $WAR_DIR/myProject-webapp-$VERSION.war --runtime-name=myProject-webapp.war --server-groups=$SERVER_GROUP_NAME_VALUE" > $JBOSS_DEPLOYMENT/deployment_myProject.log 
	cat $JBOSS_DEPLOYMENT/deployment_myProject.log | tee -a $LOG_SCRIPTS
	...
	then
		...
		printf "undeploy myProject-webapp-$OLD_VERSION.war --all-relevant-server-groups\n\n" | tee -a $LOG_SCRIPTS
		./jboss-cli.sh -c --command="undeploy myProject-webapp-$OLD_VERSION.war --all-relevant-server-groups"
